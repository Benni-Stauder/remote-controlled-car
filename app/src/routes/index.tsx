...
import useWebsocketConnector from "@/lib/websocketConnector";
...
export default function Home() {
  const {
    ...
    actions: { setConnecting, setConnected, livedata: { setSpeed, setRpm, setLateralAcceleration, setBrakePercentage}}
  } = useStore();
  const [steps, setSteps] = useState<ConnectionSteps[]>(default_steps);
  const data = useWebsocketConnector("ws://localhost:8000");
  useEffect(() => {
    setSpeed(data.speed!)
    setRpm(data.rpm!)
    setLateralAcceleration(data.lateralAcceleration!)
    setBrakePercentage(data.brakePercentage!)
  }, [data])
